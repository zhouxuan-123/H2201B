    ...
    <div class="box">
        ...
        <el-card class="box-card" style="margin-top: 10px">
          <div class="card">
            ...
            <div>
              ...
              <span
                class="span"
                v-for="(item, index) in $store.state.salarys.WagesTop"
                :key="item.id"
              >
                <el-checkbox
                  size="large"
      ...
      <div class="list">
        <el-card class="box-card" style="height: 100%">
          <el-table :data="$store.state.salarys.tableData" style="width: 100%">
          ...
          <el-pagination
            ...
            :total="$store.state.salarys.total"
      ...
      <el-dialog title="调薪" :visible.sync="dialogVisible" width="40%">
        <div class="dialog">
          ...
          <div class="foot">
            <el-form
              ref="form"
              :model="$store.state.salarys.salary"
              label-width="100px"
            >
              <el-form-item label="调整基本工资">
                <div class="form">
                  <el-input
                    v-model="$store.state.salarys.salary.currentBasicSalary"
                    disabled
                    style="width: 200px"
                  ></el-input>
              ...
              <el-form-item label="调整岗位工资">
                <div class="form">
                  <el-input
                    v-model="$store.state.salarys.salary.currentPostWage"
                    disabled
                    style="width: 200px"
                  ></el-input>
                  <span style="margin: 0 20px">-></span>
...
export default {
  ...
  methods: {
    /**请求头部数据**/
    getWagetop() {
      this.$store.dispatch('salarys/getWagetop')
    },
    /**请求表格数据**/
    getWagetable() {
      this.$store.dispatch('salarys/getWagetable', this.params)
    },
    /**分页功能**/
    handleSizeChange(val) {
      this.params.pageSize = val
      this.getWagetable(this.params)
    },
    ...
    /**过滤筛选**/
    fliter(val, index) {
      this.$store.state.salarys.WagesTop[index].checked =
        !this.$store.state.salarys.WagesTop[index].checked
      if (val.checked == true) {
        this.$store.state.salarys.tableData =
          this.$store.state.salarys._tableData.filter(
            (ele) => ele.departmentName == val.name
          )
      } else {
        this.$store.state.salarys.tableData =
          this.$store.state.salarys._tableData
      }
    },
    /**调薪**/
    changeSalary(id) {
      this.$store.dispatch('salarys/getchangeSalary', id)
